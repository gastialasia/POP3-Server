...
enum monitor_state parse_data(struct monitor_parser *p, uint8_t c)
{
  ...
  case monitor_config_add_admin:
    ...
    else if (c == 0 && p->finish_user == 0)
    { // primer separador \0 pongo el null terminated en el username
      p->monitor->data.admin_to_add.user[p->read++] = c;
      p->finish_user = 1;
      username_len = p->read;
      ret = monitor_data;
    ...
    { // Si no es alfanumerico ni fue el primer 0 separador entonces no es un dato valido
      ret = monitor_invalid_data;
      break;
    }
